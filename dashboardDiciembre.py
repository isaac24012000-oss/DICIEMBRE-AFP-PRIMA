...
st.set_page_config(layout="wide", page_icon="🏦", page_title="AFP Diciembre 2025")
...
EXCEL_PATH = os.path.join(os.getcwd(), "DATA TOTAL WORLDTEL DICIEMBRE 2025.xlsx")  # Confirmar ruta relativa
...
# CRÍTICO: PRIORIDAD contiene "14" + CONTACTABILIDAD = "Contacto Directo" + TIPO DE PAGO = "FALTA PAGO DE PAGO PLANILLAS"
# CRÍTICO: PRIORIDAD contiene "14" + CONTACTABILIDAD = "Contacto Directo" + TIPO DE PAGO = "FALTA PAGO DE PAGO PLANILLAS"
# CRÍTICO: PRIORIDAD inicia con "14", CONTACTABILIDAD tiene algún valor y REC. PLANILLAS está vacío o cero
cond_critico = (
    df_analisis['PRIORIDAD'].astype(str).str.startswith('14') &
...
# ALTO: PRIORIDAD contiene "14" pero NO es crítico
cond_alto = (
    df_analisis['PRIORIDAD'].astype(str).str.startswith('14') & (~cond_critico)
)
...
<div style='width:100%; display:flex; justify-content:center; margin:24px 0 12px 0;'>
    <div style='display:flex; gap:38px; align-items:center; background:#f7f9fc; border-radius:16px; padding:18px 32px;'>
        ...
        <span style='font-size:1.1em;'>🧨 <b>+ALTA:</b> Prioridad 14 + Contacto Directo + Sin Pago</span>
        <span style='font-size:1.1em; color:#388e3c;'>🟢 <b>ALTA:</b> Prioridad 14 (todos)</span>
...
def export_to_excel(df_export):
    ...
    # Encabezado con título
    ws['A1'] = "CASOS CRÍTICOS - PRIORIDAD 14 + CONTACTO DIRECTO + SIN PAGO"
    ws['A1'].font = Font(bold=True, size=14, color="FFFFFF")
...
<div style='display: flex; align-items: center; margin-top:32px;'>
    ...
    <h2 style='display: inline; font-size: 2rem; margin: 0; color: #c62828;'>
        Detalle de Casos Críticos (Prioridad 14 + Contacto Directo + Sin Pago)
    </h2>
</div>
""", unsafe_allow_html=True)
